<?php 
    ...
    // * finche c'e' un punto nel paragrafo    
    while (strpos($paragraph , '.') != false) {
        // # prendo l'index del primo punto (compreso)
        $index = strpos($paragraph , '.') + 1;

        // # prendo la stringa fino a quell'indice
        $sub = substr($paragraph, 0, $index);

        // # stampo in pagina
        echo '<p>' . $sub . '<br></p>';

        // # ridefinisco il paragrafo togliendo la stringa presa
        $paragraph = str_replace($sub, '', $paragraph);
    };    

    // // se vuoi una funzione fatta apposta eccola:
